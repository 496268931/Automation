...
'''
    ...
    url = 'http://api.tianma168.com/tm/getPhone?ItemId=6833&token=' + token + '&Code=UTF8'
    req = urllib2.Request(url)
    # print(req)  <urllib2.Request instance at 0x026B05A8>
...
def getMessage(token, phone, driver):
    ...
    # MSG&【哔哩哔哩】359827 为您的注册验证码，请在5分钟内完成注册。如非本人操作，请忽略或回复T退订。
    url = 'http://api.tianma168.com/tm/getMessage?token=' + token + '&itemId=6833&phone=' + phone + \
          '&Code=UTF8'
    req = urllib2.Request(url)
    data = urllib2.urlopen(req).read()
    ...
    if int == -1:
        ...
        yanzhengma = data.decode('UTF-8')[int + 3:int + 9]
        # print(yanzhengma)
        time.sleep(2)

        # 输入短信验证码
        driver.find_element_by_xpath('//*[@id="yzm"]').send_keys(yanzhengma)
        time.sleep(2)
...
def addCount(token, phone):
    ...
    headers = {
        ...
        'Referer': r'http://www.lagou.com/zhaopin/Python/?labelWords=label',
        'Connection': 'keep-alive'}
    data = {'platform': 'bilibili', 'accountId': phone, 'password': 'abc' + phone,
...
def black(token, phone):
    url = 'http://api.tianma168.com/tm/addBlack?token=' + token + '&phoneList=6833-' + phone + '&Code=UTF8'
    req = urllib2.Request(url)
    # print(req)  <urllib2.Request instance at 0x026B05A8>
    data = urllib2.urlopen(req).read()
    z_data = data.decode('UTF-8')
    ...
    for num in range(1, 51):
        try:
            ...
            while True:
                ...
                # 输入手机号
                driver.find_element_by_xpath('//*[@id="new_phone"]').send_keys(phone)
                time.sleep(2)
                # 点击免费获取短信动态码
                ...
                while True:
                    driver.find_element_by_xpath(
                        '//*[@id="login-app"]/div/div/div[3]/div/div/div[2]/div[1]/input').click()
                    time.sleep(3)
                    driver.find_element_by_xpath(
                        '//*[@id="login-app"]/div/div/div[3]/div/div/div[2]/div[1]/input').clear()
                    time.sleep(2)
                    ...
                    # 点击确定
                    driver.find_element_by_xpath(
                        '//*[@id="login-app"]/div/div/div[3]/div/div/div[2]/div[2]/input[1]').click()
                    time.sleep(5)
                    if not isElementExist('//*[@id="login-app"]/div/div/div[3]/div/div/div[2]/div['
                ...
                if (driver.find_element_by_xpath('//*[@id="registerForm"]/div[6]/p').text) == (
            ...
            # 输入昵称
            driver.find_element_by_xpath('//*[@id="registerForm"]/div[1]/input[2]').send_keys(
                phone + token[:5])
            time.sleep(2)
            ...
            # 输入密码
            driver.find_element_by_xpath('//*[@id="registerForm"]/div[3]/input').send_keys(
                'abc' + phone)
            time.sleep(2)
